...
//                println!("{:?}", entry.path());
    ...
    // init client
    let client = Client::new(config, vaults);
    println!("{:?}", client);
    ...
    let args = Cli::parse();
    match args.cmd {
        Commands::Open {vault_name, vault_group} => {
            println!("Opening vault: {:?}", vault_name);
        },
        Commands::List {vault_group} => {
            println!("Listing vaults");
        },
        Commands::Create {vault_name, vault_group} => {
            println!("Creating vault: {:?}", vault_name);
        },
        Commands::Delete {vault_name, vault_group} => {
